...
//  MoonKit
//
//  Created by Moondeer on 12/10/14.
...
public class DrawingKit : NSObject {
  ...
  private struct Cache {
    static var normalBackgroundColor: UIColor = UIColor(red: 0.529, green: 0.529, blue: 0.529, alpha: 1.000)
    static var highlightedBackgroundColor: UIColor = UIColor(red: 0.298, green: 0.298, blue: 0.306, alpha: 1.000)
    static var prominentBackgroundColor: UIColor = UIColor(red: 0.000, green: 0.463, blue: 1.000, alpha: 1.000)
    static var affirmativeGreenColor: UIColor = UIColor(red: 0.294, green: 0.847, blue: 0.384, alpha: 1.000)
  ...
  public class var normalBackgroundColor: UIColor { return Cache.normalBackgroundColor }
  public class var highlightedBackgroundColor: UIColor { return Cache.highlightedBackgroundColor }
  public class var prominentBackgroundColor: UIColor { return Cache.prominentBackgroundColor }
  public class var affirmativeGreenColor: UIColor { return Cache.affirmativeGreenColor }
  ...
  public class func drawKeyInputButton(#frame: CGRect, title: String, highlighted: Bool, prominent: Bool, reverse: Bool, disabled: Bool) {
    ...
    let normalStateColor = prominent ? DrawingKit.prominentBackgroundColor : (reverse ? DrawingKit.highlightedBackgroundColor : DrawingKit.normalBackgroundColor)
    let highlightedStateColor = prominent ? DrawingKit.highlightedBackgroundColor : (reverse ? DrawingKit.normalBackgroundColor : DrawingKit.highlightedBackgroundColor)
  ...
  public class func drawOneToneCircle(#color: UIColor, opacity: CGFloat) {
    ...
    let baseColor = UIColor(red: 0.000, green: 0.000, blue: 0.000, alpha: 1.000)
  ...
  public class func drawGreenGradientCircle(#opacity: CGFloat) {
    ...
    DrawingKit.drawGradientCircle(color1: DrawingKit.affirmativeGreenColor, color2: alpha, opacity: opacity)
    CGContextRestoreGState(context)
  }
  ...
  public class func drawSectionHeader(#rect: CGRect, titleText: String) {
    //// General Declarations
    let context = UIGraphicsGetCurrentContext()
    ...
    //// Shadow Declarations
    let textInnerShadow = NSShadow(color: UIColor.blackColor().colorWithAlphaComponent(0.6), offset: CGSizeMake(0.1, -0.1), blurRadius: 4)
    let textOuterShadow = NSShadow(color: UIColor.whiteColor(), offset: CGSizeMake(0.1, 0.6), blurRadius: 0)

    //// Rectangle Drawing
    let rectanglePath = UIBezierPath(rect: CGRectMake(rect.minX, rect.minY, rect.width, rect.height))
    tableBackgroundColor.setFill()
    rectanglePath.fill()


    //// Text Drawing
    let textRect = CGRectMake(rect.minX + 20, rect.minY + 6, rect.width - 84, rect.height - 12)
    CGContextSaveGState(context)
    CGContextSetShadowWithColor(context, textOuterShadow.shadowOffset, textOuterShadow.shadowBlurRadius, (textOuterShadow.shadowColor as UIColor).CGColor)
    let textStyle = NSMutableParagraphStyle.defaultParagraphStyle().mutableCopy() as NSMutableParagraphStyle
    textStyle.alignment = NSTextAlignment.Left

    let textFontAttributes = [NSFontAttributeName: UIFont(name: "Elysio-Medium", size: 18)!, NSForegroundColorAttributeName: tableBackgroundColor, NSParagraphStyleAttributeName: textStyle]

    let textTextHeight: CGFloat = NSString(string: titleText).boundingRectWithSize(CGSizeMake(textRect.width, CGFloat.infinity), options: NSStringDrawingOptions.UsesLineFragmentOrigin, attributes: textFontAttributes, context: nil).size.height
    let textTextRect: CGRect = CGRectMake(textRect.minX, textRect.minY + textRect.height - textTextHeight, textRect.width, textTextHeight)
    CGContextSaveGState(context)
    CGContextClipToRect(context, textRect);
    NSString(string: titleText).drawInRect(textTextRect, withAttributes: textFontAttributes)
    CGContextRestoreGState(context)
    ...
    CGContextSetShadowWithColor(context, textInnerShadow.shadowOffset, textInnerShadow.shadowBlurRadius, (textOpaqueTextShadow as UIColor).CGColor)
    CGContextSetBlendMode(context, kCGBlendModeSourceOut)
    CGContextBeginTransparencyLayer(context, nil)

    textOpaqueTextShadow.setFill()

    let textInnerShadowFontAttributes = [NSFontAttributeName: UIFont(name: "Elysio-Medium", size: 18)!, NSForegroundColorAttributeName: textInnerShadow.shadowColor!, NSParagraphStyleAttributeName: textStyle]
    NSString(string: titleText).drawInRect(textTextRect, withAttributes: textInnerShadowFontAttributes)
  ...
  public class func drawHighContrastTextBox(#frame: CGRect, txtR: CGFloat, txtG: CGFloat, txtB: CGFloat, txtA: CGFloat, bgR: CGFloat, bgG: CGFloat, bgB: CGFloat, bgA: CGFloat) {
    //// General Declarations
    let context = UIGraphicsGetCurrentContext()


    //// Shadow Declarations
    let innerShadow = NSShadow(color: UIColor.whiteColor(), offset: CGSizeMake(0.1, -0.1), blurRadius: 2)

    //// Variable Declarations
    let txtRA = txtR * txtA
    let txtRASquared = txtRA * txtRA
    let txtGA = txtG * txtA
    let txtGASquared = txtGA * txtGA
    let txtBA = txtB * txtA
    let txtBASquared = txtBA * txtBA
    let txtBrightness: CGFloat = sqrt(0.241 * txtRASquared + 0.691 * txtGASquared + 0.068 * txtBASquared)

    let bgRA = bgR * bgA
    let bgRASquared = bgRA * bgRA
    let bgGA = bgG * bgA
    let bgGASquared = bgGA * bgGA
    let bgBA = bgB * bgA
    let bgBASquared = bgBA * bgBA

    let boxBackgroundColor = UIColor(red: bgR, green: bgG, blue: bgB, alpha: bgA)
    let boxBrightness: CGFloat = sqrt(0.241 * bgRASquared + 0.691 * bgGASquared + 0.068 * bgBASquared)
    let brightnessDelta: CGFloat = boxBrightness - txtBrightness
    let boxAdjustment: CGFloat = abs(brightnessDelta) > 0.5 ? 0 : boxBrightness * 0.75
    let boxContrastColor = UIColor(red: bgR - boxAdjustment, green: bgG - boxAdjustment, blue: bgB - boxAdjustment, alpha: 1)

    //// Base Box Drawing
    let baseBoxPath = UIBezierPath(rect: CGRectMake(frame.minX, frame.minY, frame.width, frame.height))
    boxBackgroundColor.setFill()
    baseBoxPath.fill()


    //// Contrast Box Drawing
    let contrastBoxPath = UIBezierPath(rect: CGRectMake(frame.minX, frame.minY, frame.width, frame.height))
    boxContrastColor.setFill()
    contrastBoxPath.fill()

    ////// Contrast Box Inner Shadow
    CGContextSaveGState(context)
    CGContextClipToRect(context, contrastBoxPath.bounds)
    CGContextSetShadow(context, CGSizeMake(0, 0), 0)
    CGContextSetAlpha(context, CGColorGetAlpha((innerShadow.shadowColor as UIColor).CGColor))
    CGContextBeginTransparencyLayer(context, nil)
    let contrastBoxOpaqueShadow = (innerShadow.shadowColor as UIColor).colorWithAlphaComponent(1)
    CGContextSetShadowWithColor(context, innerShadow.shadowOffset, innerShadow.shadowBlurRadius, (contrastBoxOpaqueShadow as UIColor).CGColor)
    CGContextSetBlendMode(context, kCGBlendModeSourceOut)
    CGContextBeginTransparencyLayer(context, nil)

    contrastBoxOpaqueShadow.setFill()
    contrastBoxPath.fill()
    ...
    boxBackgroundColor.setStroke()
    contrastBoxPath.lineWidth = 1
    contrastBoxPath.stroke()
  }
