...
#define LogSelector NSLog(@"[%@ %@]", NSStringFromClass([self class]), NSStringFromSelector(_cmd))
...
{
//    NSLog(@"[%@ MR_importValue:%@ forKey:%@]",
//          NSStringFromClass([self class]), NSStringFromClass([value class]), key);
...
{
//    NSLog(@"[%@ MR_addObject:%@ forRelationship:%@]",
//          NSStringFromClass([self class]), NSStringFromClass([relatedObject class]), relationshipInfo.name);
...
//    LogSelector;
    for (NSString *relationshipName in relationships)
    {
        ...
        void (^establishRelationship)(NSRelationshipDescription *, id) = ^(NSRelationshipDescription *blockInfo, id blockData)
        {
            ...
            {
/*
                SEL willImportSelector = NSSelectorFromString([NSString stringWithFormat:@"willImport%@:", [relationshipName MR_capitalizedFirstCharacterString]]);
                if ([self respondsToSelector:willImportSelector])
                {
                    NSInvocation *invocation = [NSInvocation invocationWithMethodSignature:[self methodSignatureForSelector:willImportSelector]];
                    [invocation setSelector:willImportSelector];
                    [invocation setArgument:(__bridge void *)(relatedObjectData) atIndex:2];
                    [invocation invokeWithTarget:self];
                }
*/
...
/*
                SEL didImportSelector = NSSelectorFromString([NSString stringWithFormat:@"didImport%@:", [relationshipName MR_capitalizedFirstCharacterString]]);
                if ([self respondsToSelector:didImportSelector])
                {
                    NSInvocation *invocation = [NSInvocation invocationWithMethodSignature:[self methodSignatureForSelector:didImportSelector]];
                    [invocation setSelector:didImportSelector];
                    [invocation setArgument:(__bridge void *)(relatedObjectData) atIndex:2];
                    [invocation invokeWithTarget:self];
                }
*/
