...
class LearnerAppDelegate: UIResponder, UIApplicationDelegate {
  ...
  func application(application: UIApplication, didFinishLaunchingWithOptions launchOptions: [NSObject: AnyObject]?) -> Bool {
    ...
    let detectDevices: () -> Void = {
      ConnectionManager.startDetectingNetworkDevices()
    }
...
//    if wifiAvailable { detectDevices() }
    ...
    NSNotificationCenter.defaultCenter().addObserverForName(ConnectionManager.ConnectionStatusNotification,
      ...
      {
        note in
        if let wifiAvailable = note.userInfo?[ConnectionManager.WifiAvailableKey] as? Bool where wifiAvailable {
          detectDevices()
        }
