...
class NetworkDeviceMulticastConnection: NetworkDeviceConnection {
  ...
  */
  init(address a: String, port p: String, delegate: NetworkDeviceConnectionDelegate?) {
    address = a; port = p
  ...
  override var sourceFileDescriptor: dispatch_fd_t {
    ...
    if error != 0 {
      MSLogError("error getting address info for \(address), \(port): \(gai_strerror(error))")
      return -1
    }
    ...
    if socketAddress == nil || socketFileDescriptor < 0 {
      ...
      if bind(Int32(socketFileDescriptor), &socketAddress!, socketAddressLength) < 0 {
        ...
        MSLogError("failed to bind multicast socket: \(errno) - \(strerror(errno))...closing socket")
        return -1
      }
      ...
      switch Int32(socketAddress!.sa_family) {
        ...
        case AF_INET:
          ...
          error = setsockopt(socketFileDescriptor, IPPROTO_IP, IP_ADD_MEMBERSHIP, &mreq, socklen_t(sizeof(ip_mreq.self)))

        case AF_INET6:
          let sa = unsafeBitCast(socketAddress!, sockaddr_in6.self)
          var mreq6 = ipv6_mreq()
          mreq6.ipv6mr_multiaddr = sa.sin6_addr
          error = setsockopt(socketFileDescriptor, IPPROTO_IPV6, IPV6_JOIN_GROUP, &mreq6, socklen_t(sizeof(ipv6_mreq.self)))
      ...
      if error < 0 {
        ...
        MSLogError("failed to join multicast group: \(errno) - \(strerror(errno))...closing socket")
