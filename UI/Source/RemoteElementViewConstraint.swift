...
public final class RemoteElementViewConstraint: NSLayoutConstraint {
        ...
        kvoReceptionist = MSKVOReceptionist(
          ...
          handler: {
            (receptionist: MSKVOReceptionist!) -> Void in
              if let modelConstraint = receptionist.object as? Constraint {
                if let viewConstraint = receptionist.observer as? RemoteElementViewConstraint {
                  if viewConstraint.model == modelConstraint { viewConstraint.constant = CGFloat(modelConstraint.constant) }
                }
              }
            })
      }
  ...
  var valid: Bool {
    ...
    if model.multiplier != Float(multiplier) { return false }
    return true
  }
  ...
  - parameter owningView: RemoteElementView
  ...
  */
  class func constraintWithModel(model: Constraint, owningView: RemoteElementView) -> RemoteElementViewConstraint? {
    ...
    if let first = model.firstItem, view1 = owningView[first.uuid] {
      ...
      let constraint = RemoteElementViewConstraint(item: view1,
                                                   ...
                                                   multiplier: CGFloat(model.multiplier),
                                                   constant: CGFloat(model.constant))
