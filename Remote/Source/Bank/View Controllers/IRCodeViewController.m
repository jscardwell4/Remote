...
@interface IRCodeViewController ()
...
@implementation IRCodeViewController

/// initialize
+ (void)initialize {
  if (self == [IRCodeViewController class]) {
...
- (void)decorateCell:(BankItemTableViewCell *)cell forIndexPath:(NSIndexPath *)indexPath {
  ...
  __weak IRCodeViewController * weakself = self;
  ...
  if (indexPath == ManufacturerCellIndexPath) {
    ...
    cell.validationHandler = ^BOOL(BankItemTableViewCell * cell) {
    ...
    cell.changeHandler = ^(BankItemTableViewCell * cell) { weakself.irCode.frequency = cell.info; };
    ...
    cell.changeHandler = ^(BankItemTableViewCell * cell) { weakself.irCode.repeatCount = cell.info; };
