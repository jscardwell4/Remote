...
@interface REButtonEditingViewController () <MSCheckboxViewDelegate, MSTouchReporterViewDelegate, UIPickerViewDelegate>

@property (nonatomic, strong) REButtonView                   * buttonView;
...
@property (weak, nonatomic) REButton                         * buttonModel;
...
@implementation REButtonEditingViewController
@synthesize
flags,
...
+ (Class)subelementClass { return NULL; }
+ (Class)elementClass { return [REButtonView class]; }
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    if (AUTO_LAUNCH_DETAIL_EDITOR && !buttonEditorHasAutoLaunched) {
        if (REPLACE_BUTTON_COMMAND) {
            BOComponentDevice * tv =
                [BOComponentDevice fetchDeviceWithName:@"Samsung TV"
                                                    context:_buttonModel.managedObjectContext];
            RESendIRCommand * command = (RESendIRCommand *)tv.onCommand;
...
+ (void)initialize {
    if (self == [REButtonEditingViewController class]) styleNames = @[@"Custom", @"Rounded Rectangle", @"Oval", @"Rectangle", @"Triangle", @"Diamond"];
}

- (id)initWithButton:(REButton *)button
            delegate:(UIViewController <REEditingDelegate> *)delegate {
...
- (void)viewDidLoad {
    if (ValueIsNotNil(_buttonModel)) {
        self.buttonView                    = (REButtonView *)[REButtonView viewWithModel:_buttonModel];
        ...
        applyGlossCheckbox.checked = _buttonModel.style & REStyleApplyGloss;
        drawBorderCheckbox.checked = _buttonModel.style & REStyleDrawBorder;
...
- (void)setRemoteElement:(RemoteElement *)remoteElement {
    ...
    self.buttonModel = (REButton *)self.remoteElement;
}
...
- (void)checkboxValueDidChange:(MSCheckboxView *)checkbox checked:(BOOL)checked {
    ...
    if (checkbox == self.selectedStateCheckbox) {
        ...
        if (checked) _buttonModel.style |= REStyleApplyGloss;
        else _buttonModel.style &= ~REStyleApplyGloss;

        [_buttonView setNeedsDisplay];
    } else if (checkbox == self.drawBorderCheckbox) {
        if (checked) _buttonModel.style |= REStyleDrawBorder;
        else _buttonModel.style &= ~REStyleDrawBorder;
...
- (IBAction)editButtonDetails:(id)sender {
    ...
    REDetailedButtonEditingViewController * detailedEditor =
...
- (IBAction)editButtonStyle:(id)sender {
    if (_styleButton.selected) {
        ...
        NSUInteger   currentSelection = _buttonModel.style & REShapeMask;
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component {
    REStyle   style = _buttonModel.style;
