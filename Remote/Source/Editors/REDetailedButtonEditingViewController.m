...
- (void)initializeEditorWithValues:(NSDictionary *)values {
    ...
    REButton * buttonModel = NilSafeValue(values[REDetailedButtonEditingButtonKey]);
...
- (UIViewController *)childControllerForPage:(NSUInteger)pageNumber {
    ...
    switch (pageNumber) {
        case LabelEditingChildController : {
            NSString * titleText = _buttonModel.titles[_editorState.presentedState].string;
...
// kAttributeEditingTitleColorKey : CollectionSafeValue(titleColor),
        ...
        case IconEditingChildController : {
            BOIconImage * iconImage = (BOIconImage *)_buttonModel.icons[_editorState.presentedState];
            UIColor * iconColor = _buttonModel.icons.iconColors[_editorState.presentedState];
