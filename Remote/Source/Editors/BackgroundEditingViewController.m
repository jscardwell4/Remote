...
- (void)selectRowForGalleryBackgroundImage:(BOBackgroundImage *)bgImage;
- (void)resetUI;
...
- (void)resetUI {
    ...
    if (_sourceObject) {
        [self selectRowForGalleryBackgroundImage:(BOBackgroundImage *)_sourceObject.backgroundImage];
...
- (IBAction)handleTap:(UITapGestureRecognizer *)sender {
    ...
    if (sender.numberOfTapsRequired == 2 && index >= 0) {
        BOBackgroundImage * bgImage =
            (BOBackgroundImage *)self.fetchedBackgrounds[index];
...
- (void)setSourceObject:(NSManagedObject <REEditableBackground> *)sourceObject {
...
- (void)selectRowForGalleryBackgroundImage:(BOBackgroundImage *)bgImage {
    if (!bgImage) {
        ...
        NSUInteger   index =
            ...
            if (bgImage.tag == ((BOBackgroundImage *)obj).tag) {
                *stop = YES;
...
- (NSArray *)fetchedBackgrounds {
    ...
    if (!_sourceObject)
        ...
        [_sourceObject.managedObjectContext
         performBlockAndWait:^{
             NSFetchRequest * fetchRequest = [[NSFetchRequest alloc] initWithEntityName:@"BOBackgroundImage"];
             NSError * error = nil;
...
- (UITableViewCell *)tableView:(UITableView *)tableView
    ...
    if (index >= 0) {
        bgImage         = (BOBackgroundImage *)_fetchedBackgrounds[index];
