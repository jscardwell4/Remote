...
- (void)runTest {
    ...
    switch (testFocus) {
        ...
        default : MSLogWarn(DEBUG_CONTEXT, @"%@ unsupported test focus: %llu", ClassTagSelectorString, testFocus);
    }  /* switch */
...
- (void)runTranslationTestNumber:(uint64_t)testNumber options:(uint64_t)testOptions {
    ...
    switch (testNumber) {
        ...
        default :
            MSLogWarn(DEBUG_CONTEXT, @"%@ unsupported test number:%llu", ClassTagSelectorString, testNumber);
            break;
    } /* switch */
}     /* runTranslationTestNumber */
...
- (void)runAlignmentTestNumber:(uint64_t)testNumber options:(uint64_t)testOptions {
    ...
    MSRemoteUITestRunner   performAlignments = ^(NSDictionary * parameters) {
        ...
        for (NSString * alignment in alignments) {
            ...
            [MainQueue addOperations:@[BlockOperation(bgEditor.focusView = nil;
            ...
            SuppressWarning("-Warc-performSelector-leaks",
                            [MainQueue addOperations:@[BlockOperation([bgEditor performSelector:selector withObject:nil];)]
                                   waitUntilFinished:YES];)
    ...
    switch (testNumber) {
        case 0 :{    // vertical alignment
            performAlignments(@{
                                  ...
                                  MSRemoteUIAssertionsKey : @[^(NSDictionary * variables) {
                                                              ...
                                                              ^(NSDictionary * variables)
                {
                    ...
                    assert(digit0);
                    NSDictionary * values = @{
                        @"firstItem" : aux1.identifier,
                        @"firstAttribute" : @(NSLayoutAttributeBottom),
                        @"relation" : @(NSLayoutRelationEqual),
                        @"secondItem" : digit0.identifier
                    };
                    @try {NSAssert(![aux1.firstItemConstraints
                                     firstObjectPassingTest:^BOOL (RemoteElementLayoutConstraint * obj, BOOL * stop) {
                                    return ([obj hasAttributeValues:values] && (*stop = YES));
                                }], @"constraint binding aux1.bottom to digit0.bottom should have been removed and was not"); } @catch (NSException * exception) {}
                }]
                              });
        }   break;
...
- (void)runInfoTestNumber:(uint64_t)testNumber options:(uint64_t)testOptions {
    MSRemoteUITestRunner   dumpInfo = ^(NSDictionary * parameters) {
        ...
        if (!_flags.quietMode) {
            ...
            [self logView:bgv after:0.5 message:nil];
        }
    };
    ...
    switch (testNumber) {
        ...
        case 1 : dumpInfo(@{MSRemoteUIRemoteKey : @"activity1",
                            MSRemoteUIButtonGroupKey : kTopPanelOneKey});
            break;
        default : MSLogWarn(DEBUG_CONTEXT,
