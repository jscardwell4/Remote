 ...
 */
@interface RECommand : MSModelObject <MSNamedModelObject>
 ...
 * @return The newly created `HTTPCommand` object.
 */
+ (REHTTPCommand *)commandWithURL:(NSString *)url context:(NSManagedObjectContext *)context;
...
MSKIT_STATIC_INLINE Class classForCommandImportType(NSString * type)
{
    ...
    dispatch_once(&onceToken, ^{
        index = @{ @"power"    : [REPowerCommand    class],
                   @"ir"       : [RESendIRCommand   class],
                   @"http"     : [REHTTPCommand     class],
                   @"delay"    : [REDelayCommand    class],
                   @"activity" : [REActivityCommand class] };
    });
    return index[type];
}
