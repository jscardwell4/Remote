...
- (void)awakeFromNib
{
    ...
    [self registerForNotificationCenterNotifications];
                         assert([RESystemCommand registerRemoteViewController:self]);
}
...
- (void)dealloc
{
    ...
    [RESystemCommand registerRemoteViewController:nil];
}
...
{
    _topToolbar = [REButtonGroupView viewWithModel:
                   (REButtonGroup *)[_remoteController.topToolbar faultedObject]];
    assert(_topToolbar);
