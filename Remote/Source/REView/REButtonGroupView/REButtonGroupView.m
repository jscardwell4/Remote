...
MSKIT_STRING_CONST   REButtonGroupTuckButtonKey       = @"REButtonGroupTuckButtonKey";
MSKIT_STRING_CONST   REButtonGroupViewPanelNametag    = @"REButtonGroupViewPanel";
MSKIT_STRING_CONST   REButtonGroupViewInternalNametag = @"REButtonGroupViewInternal";
...
- (void)updateConstraints
{
    ...
    if (![self constraintsWithNametagPrefix:REButtonGroupViewInternalNametag])
    {
        NSString * constraints =
              ...
              $(@"%@-%@", REButtonGroupViewInternalNametag, REButtonGroupViewLabelNametag));
...
- (void)initializeViewFromModel {
    ...
    _isPanel = (self.model.panelLocation != REPanelLocationNotAPanel);
}
...
- (void)didMoveToSuperview {
    ...
    if (self.superview && _isPanel && !self.isEditing)
    {
        ...
        _tuckGesture.nametag = $(@"'%@'-tuck", self.displayName);
        _tuckGesture.enabled = NO;
        _untuckGesture = [[MSSwipeGestureRecognizer alloc] initWithTarget:self
                                                                   action:@selector(handleSwipe:)];
        _untuckGesture.nametag = $(@"'%@'-untuck", self.displayName);
        ...
        _tuckedConstraint = [NSLayoutConstraint constraintWithItem:self
                                                        ...
                                                        multiplier:1.0f
                                                          constant:0.0f];
        _tuckedConstraint.priority = 999;
        _tuckedConstraint.nametag = REButtonGroupViewPanelNametag;
        _untuckedConstraint = [NSLayoutConstraint constraintWithItem:self
                                                          ...
                                                          multiplier:1.0f
                                                            constant:0.0f];
        _untuckedConstraint.priority = 1;
        _untuckedConstraint.nametag = REButtonGroupViewPanelNametag;
...
- (void)addSubelementView:(REButtonView *)view
{
    ...
    if ([view.key isEqualToString:REButtonGroupTuckButtonKey])
    {
...
- (REButtonView *)objectAtIndexedSubscript:(NSUInteger)idx {
    return (REButtonView *)[super objectAtIndexedSubscript:idx];
}

- (REButtonView *)objectForKeyedSubscript:(NSString *)key {
    return (REButtonView *)[super objectForKeyedSubscript:key];
}

@end

@implementation REButtonGroupView (Drawing)

- (void)drawRoundedPanelInContext:(CGContextRef)ctx inRect:(CGRect)rect
{
    REButtonGroupPanelLocation   panelLocation = self.panelLocation;
