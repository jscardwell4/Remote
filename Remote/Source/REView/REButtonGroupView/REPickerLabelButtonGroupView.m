//
// REPickerLabelButtonGroupView.m
// Remote
//
...
MSKIT_STRING_CONST
REPickerLabelButtonGroupViewInternalNametag = @"REPickerLabelButtonGroupViewInternal";

MSKIT_STRING_CONST
REPickerLabelButtonGroupViewLabelContainer = @"REPickerLabelButtonGroupViewLabelContainer";
    ...
    if (![self constraintsWithNametagPrefix:REPickerLabelButtonGroupViewInternalNametag])
    {
        ...
        $(@"'%1$@' _labelContainer.centerY = self.centerY\n"
          ...
          $(@"%@-%@",
            ...
            REPickerLabelButtonGroupViewLabelContainer));
...
- (NSDictionary *)kvoRegistration
{
    return [[super kvoRegistration]
            ...
            @{@"labels" : MSMakeKVOHandler({[(__bridge REPickerLabelButtonGroupView *)context
                                             buildLabels];}),
              @"commandSets" : MSMakeKVOHandler({[(__bridge REPickerLabelButtonGroupView *)context
                                                  updateCommandSet];})}];
}
...
- (void)buildLabels
{
    ...
    RECommandSetCollection * collection = ((REPickerLabelButtonGroup*)self.model).commandSetCollection;

    NSMutableArray * labels = [@[] mutableCopy];
    for (RECommandSet * commandSet in collection.commandSets)
...
- (void)updateCommandSet
{
    ...
    RECommandSet * commandSet = commandSets[_pickerFlags.labelIndex];
    assert(commandSet);

    for (REButton * button in self.subelements)
        if ([commandSet isValidKey:button.key]) button.command = commandSet[button.key];
}
