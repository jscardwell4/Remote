...
- (void)updateState {
    ...
    if (_bvflags.state != currentState) {
        if (   (_bvflags.state & UIControlStateDisabled)
            != (currentState & UIControlStateDisabled))
            self.userInteractionEnabled = !self.userInteractionEnabled;

        _labelView.attributedText = [self.model titleForState:currentState];
        _icon                     = [self.model iconUIImageForState:currentState];
...
- (UIControlState)state { return _bvflags.state; }
...
- (void)buttonActionWithOptions:(CommandOptions)options {
    ...
    if (!self.editing && _bvflags.commandsActive) {
        ...
        [self.model executeCommandWithOptions:options delegate:self];
    }
}
...
- (void)initializeViewFromModel {
    ...
    _bvflags.activityIndicator = [self.model.command isKindOfClass:[MacroCommand class]];

    _labelView.attributedText = [self.model titleForState:_bvflags.state];
    _icon                     = [self.model iconUIImageForState:_bvflags.state];
    [self invalidateIntrinsicContentSize];
    [self setNeedsDisplay];
}

- (NSDictionary *)kvoRegistration {
    __weak REButtonView * weakSelf        = self;
    __strong NSDictionary  * kvoRegistration =
    @{
        @"selected"    : ^(MSKVOReceptionist * r, NSString * k, id o, NSDictionary * c, void * ctx)
                         { [weakSelf updateState]; },
        @"enabled"     : ^(MSKVOReceptionist * r, NSString * k, id o, NSDictionary * c, void * ctx)
                         { [weakSelf updateState]; },
        @"highlighted" : ^(MSKVOReceptionist * r, NSString * k, id o, NSDictionary * c, void * ctx)
                         { [weakSelf updateState]; },
        @"command"     : ^(MSKVOReceptionist * r, NSString * k, id o, NSDictionary * c, void * ctx)
                         { _bvflags.activityIndicator =
                             [self.model.command isKindOfClass:[MacroCommand class]]; },
        @"style"       : ^(MSKVOReceptionist * r, NSString * k, id o, NSDictionary * c, void * ctx)
                         { [weakSelf setNeedsDisplay]; },
        @"titles"      : ^(MSKVOReceptionist * r, NSString * k, id o, NSDictionary * c, void * ctx)
                         { _labelView.attributedText = [weakSelf.model titleForState:_bvflags.state]; }
...
- (UIColor *)backgroundColor {
    return [self.model backgroundColorForState:_bvflags.state];
}
...
- (void)initializeIVARs {
    ...
    _frameIcon      = [REIconImage
                       fetchIconWithTag:kFrameTag
                                context:self.model.managedObjectContext];
    [self.model setIcon:self.frameIcon forState:kFrameState];
    [self.model setIconColor:self.frameColor forState:kFrameState];
    _plugIcon = [REIconImage
                 fetchIconWithTag:kPlugTag
                          context:self.model.managedObjectContext];
    [self.model setIcon:self.plugIcon forState:kPlugState];
    [self.model setIconColor:self.plugColor forState:kPlugState];
    _lightingIcon = [REIconImage
                     fetchIconWithTag:kLightningTag
                              context:self.model.managedObjectContext];
    [self.model setIcon:self.lightingIcon forState:kLightningState];
    [self.model setIconColor:self.lightningColor forState:kLightningState];
    ...
    __weak REBatteryStatusButtonView * bsBV = self;
...
- (CGSize)intrinsicContentSize {
    UIImage * iconImage = [self.model iconUIImageForState:kFrameState];
...
- (void)drawContentInContext:(CGContextRef)ctx inRect:(CGRect)rect {
    ...
    UIImage * frameImage = [self.model iconUIImageForState:kFrameState];
    ...
    if (_batteryState == UIDeviceBatteryStateFull) {
        UIImage * plugImage = [self.model iconUIImageForState:kPlugState];

        [plugImage drawInRect:CGRectInset(frameRect, padding, padding)];
    } else if (_batteryState == UIDeviceBatteryStateCharging) {
        UIImage * lightningImage = [self.model iconUIImageForState:kLightningState];
...
- (void)initializeIVARs {
    if (![self.model iconUIImageForState:UIControlStateNormal]) {
        [self.model setIcon:[REIconImage fetchIconWithTag:kDefaultIconTag
                                                 context:self.model.managedObjectContext]
             forState:UIControlStateNormal];
        [self.model setIconColor:GrayColor forState:UIControlStateNormal];
        [self.model setIconColor:WhiteColor forState:UIControlStateSelected];
        [self invalidateIntrinsicContentSize];
    }

    self.selected = [[ConnectionManager sharedConnectionManager] isWifiAvailable];

    __weak REConnectionStatusButtonView * csBV = self;
    ...
    [NotificationCenter addObserverForName:kConnectionStatusNotification
                                ...
                                usingBlock:^(NSNotification * note) {
                                    ...
                                    if (csBV.selected != connected) ((REButton *)csBV.model).selected = connected;
                                }
...
- (CGSize)intrinsicContentSize {
    UIImage * iconImage = [self.model iconUIImageForState:UIControlStateNormal];
