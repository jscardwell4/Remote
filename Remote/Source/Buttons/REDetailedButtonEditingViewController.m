...
@interface REDetailedButtonEditingViewController () <UIGestureRecognizerDelegate>
@property (nonatomic, weak) REButton                      * buttonModel;
...
@property (weak, nonatomic) REButtonEditingViewController * buttonEditor;
...
@implementation REDetailedButtonEditingViewController

- (void)initializeEditorWithValues:(NSDictionary *)values {
    if (ValueIsNil(values)) return;

    _editorState.presentedState =
        [values[kDetailedButtonEditingControlStateKey] unsignedIntegerValue];

    REButton * buttonModel = NilSafeValue(values[kDetailedButtonEditingButtonKey]);
...
- (UIViewController *)childControllerForPage:(NSUInteger)pageNumber {
    ...
    switch (pageNumber) {
        case LabelEditingChildController : {
            NSString * titleText = [_buttonModel titleForState:_editorState.presentedState].string;
...
// kAttributeEditingTitleColorKey : CollectionSafeValue(titleColor),
        ...
        case IconEditingChildController : {
            REIconImage * iconImage =
                [_buttonModel iconImageForState:_editorState.presentedState];
            UIColor * iconColor =
                [_buttonModel iconColorForState:_editorState.presentedState];
...
- (void)willMoveToParentViewController:(UIViewController *)parent {
    if ([parent isMemberOfClass:[REButtonEditingViewController class]]) self.buttonEditor = (REButtonEditingViewController *)parent;
...
- (void)setRemoteElement:(REButton *)remoteElement {
    if ([remoteElement isKindOfClass:[REButton class]]) {
