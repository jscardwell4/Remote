...
- (void)viewDidLoad {
    if (ValueIsNotNil(_buttonModel)) {
        ...
        applyGlossCheckbox.checked = _buttonModel.style & REButtonStyleApplyGloss;
        drawBorderCheckbox.checked = _buttonModel.style & REButtonStyleDrawBorder;
...
- (void)checkboxValueDidChange:(MSCheckboxView *)checkbox checked:(BOOL)checked {
    ...
    if (checkbox == self.selectedStateCheckbox) {
        ...
        if (checked) _buttonModel.style |= REButtonStyleApplyGloss;
        else _buttonModel.style &= ~REButtonStyleApplyGloss;

        [_buttonView setNeedsDisplay];
    } else if (checkbox == self.drawBorderCheckbox) {
        if (checked) _buttonModel.style |= REButtonStyleDrawBorder;
        else _buttonModel.style &= ~REButtonStyleDrawBorder;
...
- (IBAction)editButtonStyle:(id)sender {
    if (_styleButton.selected) {
        ...
        NSUInteger   currentSelection = _buttonModel.style & REButtonShapeMask;
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component {
    REButtonStyle   style = (REButtonStyle)_buttonModel.style;

    style             &= REButtonStyleMask;
    style             |= row;
