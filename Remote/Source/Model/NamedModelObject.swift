...
extension NamedModelObject: NamedModel, RenameableModel {

  /** save */
  func save() { if let moc = managedObjectContext { CoreDataManager.saveContext(moc, propagate: true) } }

  /** delete */
  func delete() {
    if let moc = self.managedObjectContext {
      moc.performBlockAndWait { moc.processPendingChanges(); moc.deleteObject(self) }
      CoreDataManager.saveContext(moc, propagate: true)
    }
  }
  ...
  /** willSave */
  override public func willSave() { if !deleted { _ = name } }
  ...
  */
  override public func updateWithData(data: [NSObject : AnyObject]!) {
  ...
  */
  ...
  class var existingNames: NSSet {
    ...
    if names!.count == 0 {
      ...
      var error: NSError?
      let moc = CoreDataManager.defaultContext()
