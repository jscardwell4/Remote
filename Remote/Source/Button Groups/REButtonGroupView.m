...
- (CGSize)sizeThatFits:(CGSize)size
{
    ...
    CGSize   fittedSize     = CGSizeMake(MAX(minimumSize.width, childUnionSize.width),
                                         MAX(minimumSize.height, childUnionSize.height));
...
- (void)buttonViewDidExecute:(REButtonView *)buttonView
{
...
- (void)tuckAction:(id)sender
{
    if (self.parentElementView)
        [(RERemoteView*)self.parentElementView tuckRequestFromButtonGroupView : self];
}
...
- (NSDictionary *)kvoRegistration
{
    __strong NSDictionary * kvoRegistration =
        ...
        {
            id   newValue = c[NSKeyValueChangeNewKey];
...
- (void)addSubelementView:(REButtonView *)view
{
    ...
    {
        __weak REButtonGroupView * weakself = self;
...
- (void)setEditingMode:(EditingMode)mode
{
    ...
    if (self.editingMode == EditingModeEditingRemote)
        self.buttonsEnabled = NO;
    
    else if (self.editingMode == EditingModeEditingButtonGroup)
...
- (void)setResizable:(BOOL)resizable
{
    ...
    if (RESIZABLE_TRICKLES_DOWN)
        for (REButtonView * buttonView in self.subelementViews)
...
@implementation RERoundedPanelButtonGroupView {}
...
{
    REButtonGroupSubtype   panelLocation = self.panelLocation;

    CGContextClearRect(ctx, self.bounds);

    NSUInteger   roundedCorners = (panelLocation == REButtonGroupPanelLocationRight
                                   ? UIRectCornerTopLeft | UIRectCornerBottomLeft
                                   : (panelLocation == REButtonGroupPanelLocationLeft
    ...
    CGFloat   tx        = (panelLocation == REButtonGroupPanelLocationRight ? 3 : -3);
...
@implementation RESelectionPanelButtonGroupView {
     ...
     */
    __weak REButtonView * _selectedButton;
}
...
- (void)setSelectedButton:(REButtonView *)newSelection
{
    if (_selectedButton != newSelection)
    {
        if (_selectedButton)
            _selectedButton.model.selected = NO;
...
@implementation REPickerLabelButtonGroupView {
    struct {
        BOOL         blockPan;
        CGFloat      panLength;
...
- (NSDictionary *)kvoRegistration
{
    __weak REPickerLabelButtonGroupView * weakSelf = self;
...
- (void)buildLabels
{
    ...
    NSOrderedSet * labelSet = ((REPickerLabelButtonGroup*)self.model).commandSetLabels;
...
{
    NSURL * uri = [self.model.commandSets objectAtIndex: _pickerFlags.labelIndex];
    NSManagedObjectID * objectID = [[self.model.managedObjectContext persistentStoreCoordinator]
                                    managedObjectIDForURIRepresentation:uri];

    if (objectID)
    {
        CommandSet * commandSet = (CommandSet*)[self.model.managedObjectContext objectWithID:objectID];

        if (commandSet)
            for (REButton * button in self.subelements)
                if ([commandSet isValidKey:button.key])
                    button.command = [commandSet commandForKey:button.key];
