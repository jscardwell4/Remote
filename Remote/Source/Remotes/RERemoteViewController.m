...
@implementation RERemoteViewController {
    struct {
    ...
    REButtonGroupView    * _topToolbar;
    NSLayoutConstraint * _topToolbarConstraint;
    RERemoteController   * _remoteController;
...
- (void)observeValueForKeyPath:(NSString *)keyPath
    ...
    MSRunSyncOnMain (^{[self insertRemoteView:(RERemoteView *)[REView viewWithModel:(RERemote *)change[NSKeyValueChangeNewKey]]]; }
...
- (void)viewDidLoad {
    ...
    [self insertRemoteView:(RERemoteView *)[REView
                                          viewWithModel:_remoteController.currentRemote]];
...
- (void)updateViewConstraints {
    ...
    RERemoteView * remoteView = (RERemoteView *)[self.view viewWithNametag:kRemoteViewNameTag];
...
- (void)insertRemoteView:(RERemoteView *)remoteView {
    assert(OnMainThread && remoteView);

    RERemoteView * currentRV = (RERemoteView *)[self.view viewWithNametag:kRemoteViewNameTag];

    assert([currentRV isKindOfClass:[RERemoteView class]] || currentRV == nil);
...
- (void)initializeTopToolbar {
    _topToolbar = (REButtonGroupView *)[REView viewWithModel:(REButtonGroup *)
