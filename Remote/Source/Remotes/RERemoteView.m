...
@implementation RERemoteView {
    ...
    __weak REButtonGroupView * _overlayGroupCurrentlyDisplayed;
}
...
{
    REButtonGroupView * panel = [self panelForKey:panelKey];

    if (ValueIsNil(panel))
    {
        DDLogWarn(@"%@\n\tno button group registered for panel:%@",
                  ClassTagSelectorStringForInstance(self.displayName), panelKey);
...
- (BOOL)buttonGroupViewIsPanel:(REButtonGroupView *)buttonGroupView
{
    return ([_buttonGroupPanelAssignments objectForKey:buttonGroupView.key] != nil);
}

- (void)registerView:(REButtonGroupView *)buttonGroupView forPanel:(NSString *)panelKey
{
...
- (void)unregisterView:(REButtonGroupView *)buttonGroupView forPanel:(NSString *)panelKey
{
    if (  [kValidPanelKeys containsObject:panelKey]
...
- (void)tuckRequestFromButtonGroupView:(REButtonGroupView *)buttonGroupView
{
    if (  [self buttonGroupViewIsPanel:buttonGroupView]
       && [_currentlyDisplayedPanel isEqualToString:buttonGroupView.key])
        [self tuckPanelForKey:_currentlyDisplayedPanel];
}
        ...
        dispatch_after(popTime,
                       ...
                       ^(void){
                           for (NSString * panelKey in kValidPanelKeys)
                               if ([_buttonGroupPanelAssignments objectForKey:panelKey] != nil)
                                   [self tuckPanelForKey:panelKey];
                       });
    }
}
...
- (void)addSubelementView:(REButtonGroupView *)view
{
...
- (void)removeSubelementView:(REButtonGroupView *)view
{
...
- (void)setEditingMode:(EditingMode)mode
{
    ...
    else if (self.editingMode == EditingModeEditingRemote)
        [self updateActiveGestures:YES];
