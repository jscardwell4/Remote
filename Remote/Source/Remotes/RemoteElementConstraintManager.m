...
+ (void)initialize {
    if (self == [RemoteElementConstraintManager class])
    {
        ...
        kConstraintPropertyKeys = [RemoteElementLayoutConstraint propertyList];
    }
}

+ (RemoteElementConstraintManager *)constraintManagerForRemoteElement:(RemoteElement *)element {
    RemoteElementConstraintManager * manager = [RemoteElementConstraintManager new];
     ...
     ^{ // wrap method in managed object context block
         ...
         for (RemoteElement * element in subelements)
         {
             ...
             [self removeProportionLockForElement:element
                                      currentSize:CGRectValue(metrics[element.identifier]).size];
             ...
             RemoteElementLayoutConstraint * c = [RemoteElementLayoutConstraint
     ...
     ^{ // wrap method in managed object context block
         ...
         {// freeze sibling constraints dependent on element, freeze element size, add new constraint
          // adjust constraints that depend on the view being moved
             ...
             [self freezeSize:CGRectValue(metrics[element.identifier]).size
...
- (void)shrinkWrapSubelements:(NSDictionary *)metrics
{
    ...
    [metrics dictionaryWithValuesForKeys:
     [[metrics allKeys]
      ...
      _remoteElement.parentElement.identifier]];
    ...
    CGSize    currentSize = CGRectValue(metrics[_remoteElement.identifier]).size;
    ...
    CGSize  boundingSize =
        CGRectValue(metrics[_remoteElement.parentElement.identifier]).size;
     ...
     ^{ // wrap method in managed object context block
         if (element.layoutConfiguration.proportionLock)
         {
             RemoteElementLayoutConstraint * c =
                 [element.intrinsicConstraints firstObjectPassingTest:
     ...
     ^{ // wrap method in managed object context block
         for (RemoteElementLayoutConstraint * constraint in _remoteElement.dependentChildConstraints)
         {
             if (constraint.multiplier != 1.0f)
             {
                 ...
                 CGRect frame = CGRectValue(metrics[constraint.firstItem.identifier]);
                 CGRect bounds = (CGRect){.
                     size = CGRectValue(metrics[_remoteElement.identifier]).size
                 };
                 ...
                 [_remoteElement addConstraint:[RemoteElementLayoutConstraint
    ...
    [_context performBlockAndWait:
     ^{ // wrap method in managed object context block
         for (RemoteElementLayoutConstraint * constraint in constraints)
         {
             ...
             CGRect bounds = (CGRect){.size = CGRectValue(metrics[_remoteElement.identifier]).size};
             CGRect frame  = CGRectValue(metrics[constraint.firstItem.identifier]);
             ...
             [owner addConstraint:
              [RemoteElementLayoutConstraint constraintWithAttributeValues:constraintValues]];
         }
     }];
}
         ...
         attribute:(NSLayoutAttribute)attribute
{
    ...
    [_context performBlockAndWait:
     ...
     for (RemoteElementLayoutConstraint * constraint in constraintsToRemove)
         [constraint.owner removeConstraint:constraint];

     [subelement addConstraint:[RemoteElementLayoutConstraint
                                constraintWithItem:subelement
                                attribute:firstAttribute
                                relatedBy:NSLayoutRelationEqual
                                toItem:nil
                                attribute:NSLayoutAttributeNotAnAttribute
                              ...
                              metrics:(NSDictionary *)metrics
{
    [_context performBlockAndWait:
     ^{ // wrap method in managed object context block
         ...
         CGRect frame = CGRectValue(metrics[constraint.firstItem.identifier]);
         CGRect bounds = (CGRect){
             .size = CGRectValue(metrics[constraint.firstItem.parentElement.identifier]).size
         };

         for(RemoteElementLayoutConstraint * constraint in removal)
             [constraint.owner removeConstraint:constraint];
        ...
        for (NSNumber * n in additions)
        {
            ...
            [owner addConstraint:[RemoteElementLayoutConstraint constraintWithItem:firstItem
                                                                         attribute:firstAttribute
                                                                         relatedBy:NSLayoutRelationEqual
                                                                            toItem:secondItem
